...
import { download,Delete as delete_ } from '../assets';
import { downloadImage } from '../utils/index.js'
import { Navigate } from 'react-router-dom';

const Card = ({_id,name,prompt,photo,userName,deleteimage}) => {
    ...
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card'>
      ...
      <div className='group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 
        ...
        <div className='mt-5 flex justify-between items-center gap-2'>
          ...
          <button type='button' onClick={()=>downloadImage(_id,photo)} className='outline-none border-none bg-transparent ml-auto'>
            <img className='w-6 h-6 object-contain invert' src={download} alt="download"/>
          </button>
          <button type='button' onClick={()=>deleteimage(_id)}  className='outline-none border-none bg-transparent'>
            <img className='w-6 h-6 object-contain invert' src={delete_} alt="delete"/>
          </button>
