...
const CreatePost = ({userName}) => {
  ...
  const generateImage = async ()=>{
    if(form.prompt){
      try{
        ...
        const response = await fetch('https://imagegenie-oyia.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) =>{
    ...
    if (form.prompt && form.photo){
      setLoading(true);
      try{
        const response = await fetch('https://imagegenie-oyia.onrender.com/api/v1/post/add', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <form  className='max-w-3xl mt-16' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          <div className='relative flex items-center justify-center w-64 h-64 p-3 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500'>
            { form.photo ?
              <img src={form.photo} alt={form.name} className='object-contain w-full h-full'/>
              :
        ...
        <div className='mt-10'>
            <p className='mt-2 text-[#666e75] text-[14px]'>Once you have created the image you want,you can add it to Gallery</p>
