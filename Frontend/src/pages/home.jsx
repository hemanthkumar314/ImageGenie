...
const Home = ({userName}) => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true);
      try {
        const response = await fetch('https://imagegenie-oyia.onrender.com/api/v1/post/all', {
          method: 'POST',
          headers: {
  ...
  const deleteimage = async (_id) => {
    try {
      const response = await fetch('https://imagegenie-oyia.onrender.com/api/v1/post/deleteimg', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <section className='items-center mx-auto my-4 max-w-7xl ml-14'>
      <div>
